...
export default function Login(){
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('userRole', res.data.user?.role || 'nasabah'); // 👈 Simpan role
      navigate('/dashboard');
    } catch (err) {
      ...
      <div className="max-w-5xl w-full bg-white rounded-2xl shadow-2xl overflow-hidden grid grid-cols-1 md:grid-cols-2">
        ...
        <div className="p-12 py-16 flex flex-col justify-center">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 text-simgreen-600 border-gray-300 rounded focus:ring-simgreen-500" />
                Ingat saya
              </label>
              <a className="text-sm text-simgreen-600 hover:text-simgreen-700 font-medium" href="#">Lupa password?</a>
            </div>

            <button 
              type="submit" 
              disabled={loading} 
              className="w-full bg-gradient-to-r from-simgreen-600 to-simgreen-500 hover:from-simgreen-700 hover:to-simgreen-600 text-white font-semibold py-3 rounded-lg transition shadow-md hover:shadow-lg flex items-center justify-center gap-2"
            >
          ...
          <div className="mt-6 text-center">
            ...
            <a 
              className="text-simgreen-600 hover:text-simgreen-700 font-medium text-sm inline-flex items-center gap-1" 
              href="/register"
            >
          ...
          <div className="mt-6 pt-6 border-t border-gray-200 text-center">
            <div className="text-xs text-gray-500 mb-2">Butuh bantuan?</div>
            <a href="#" className="text-sm text-simgreen-600 hover:text-simgreen-700 font-medium">Hubungi support</a>
          </div>
        </div>
      </div>
    </div>
